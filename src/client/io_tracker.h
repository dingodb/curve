...
public:
  IOTracker(IOManager *iomanager, MetaCache *mc, RequestScheduler *scheduler,
            FileMetric *clientMetric = nullptr, bool disableStripe = false);
   ...
   * @param: scc是异步回调
   */
  void ReadSnapChunk(const ChunkIDInfo &cinfo, uint64_t seq, uint64_t offset,
   ...
   */
  void GetChunkInfo(const ChunkIDInfo &cinfo, ChunkInfoDetail *chunkInfo);
  ...
  bool IsStripeDisabled() const { return disableStripe_; }
